...
The historical final50 cohort was cleared under earlier explicit authorization. The user now authorizes cleanup of failed games: use harness cleanup, retain compact failure evidence and all referenced dependencies, and protect passing/accepted games. The arcade uses a paginated library index and labels historical/development previews separately from factory acceptance. Search, focus and scroll survive returning from play. Advanced generation options stay below the player workflow.
...
Historical pilot readiness is superseded by the archive checkpoint below.

The current G03 review request is generated by `g03-review-request` from the
active candidate index and evidence manifest. Its source hash and candidate IDs
...
Retired experiment storage is now the eight-file packet `.agent/archives/built-unproven/`: two compressed restoration shards, factual JSONL, family-summary JSONL with model receipts, lessons, manifest, README and final brief. Expanded originals were removed after full restoration and all 64,640 file hashes were verified. Root manifest maps ordered archive shards and analysis hashes; the original full manifest is inside the archive under `original-metadata/`. Archive binary shards stay local and Git-ignored. Model summaries are hypotheses, not acceptance evidence; rejected draft inferences remain labeled. Active arcade stays empty, G01–G33 identities remain unchanged, and prior readiness still requires revalidation. Restore into an isolated directory and use independent copies for edits because original runtime hard links are preserved.
