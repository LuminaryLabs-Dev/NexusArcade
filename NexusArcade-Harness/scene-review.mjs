...
export async function reviewScene(root,id,plan,{signal}={}){
 ...
 try{
  ...
  async function playerDetail(frameId){
   if(c.presentation.camera==='forward')return null;
   const e=await page.evaluate(()=>__renderEvidence()),b=e.actorScreenBounds;
   if(!b||![b.left,b.right,b.top,b.bottom].every(Number.isFinite)||b.left<0||b.right>1100||b.top<0||b.bottom>780)throw Error('Player detail lacks an on-screen subject');
   const clip={x:Math.max(0,Math.min(880,(b.left+b.right)/2-110)),y:Math.max(0,Math.min(560,(b.top+b.bottom)/2-110)),width:220,height:220};
   const bytes=await page.screenshot({clip});(report.playerDetails??=[]).push({frameId,clip,actorScreenBounds:b,playerMarker:e.playerMarker,imageHash:digest(bytes)});return bytes;
  }
  ...
  const render=await page.evaluate(()=>__renderEvidence());check('all requested presenters instantiated',render.presenters.length===c.presentation.nodes.length);check('Three.js scene renders',render.triangles>1000);if(c.presentation.camera!=='forward')check('player identified at start',render.playerMarker?.visible===true);if(c.presentation.camera==='overhead')check('overview names every valve at start',c.presentation.nodes.filter(n=>n.capability==='valve').every(n=>render.domainViews[n.id]?.labelVisible===true));report.initialImage=await page.screenshot();report.initialHash=digest(report.initialImage);report.initialPlayerImage=await playerDetail('initial');
  async function walk(x,z){const s=await state(),world=c.runtime.collision.world,route=routeInWorld(world,s.player,{x,z},s.domainState);for(const waypoint of route.points.slice(1)){let reached=false;for(let i=0;i<700;i++){const s=await state(),dx=waypoint.x-s.player.x,dz=waypoint.z-s.player.z;if(Math.hypot(dx,dz)<.18){reached=true;break;}if(s.mode==='won')return;if(s.mode!=='play')throw Error('Session ended before planned movement');await input([Math.abs(dx)>.1?(dx>0?'KeyD':'KeyA'):null,Math.abs(dz)>.1?(dz>0?'KeyS':'KeyW'):null].filter(Boolean));const next=await state();if(!worldSupports(world,next.player,next.domainState))throw Error('Actor left valid world');}if(!reached)throw Error('Planned waypoint unreachable through movement');}}
  async function run(omit=false){for(const step of plan.steps){if((await state()).mode!=='play')break;if(step.action==='move')await walk(step.x,step.z);if(step.action==='interact'&&!omit)for(let i=0;i<step.count;i++){await input(['KeyE']);await tick(50);if(!report.image&&(await state()).mode==='play'){report.image=await page.screenshot();report.frame=await state();report.playerImage=await playerDetail('consequence');}}if(step.action==='wait'){for(let left=step.seconds;left>0&&(await state()).mode==='play';left-=.25){await tick(Math.min(.25,left)*1000);if(!report.image&&!omit){report.image=await page.screenshot();report.frame=await state();report.playerImage=await playerDetail('consequence');}}}}return state();}
  ...
  report.runs=[{mode:won.mode,elapsed:won.elapsed,result:won.lastResult}];check('record saved',won.bestSeconds===won.elapsed);if(!report.image){report.image=await page.screenshot();report.frame=won;report.playerImage=await playerDetail('consequence');}
  ...
  if(plan.steps.some(s=>s.action==='interact')){await page.keyboard.press('KeyR');await tick(0);try{await run(true);}catch(e){if(e.message!=='No traversable route')throw e;report.omittedRouteBlocked=true;}await tick((c.runtime.session.durationSeconds+1)*1000);check('omitted interaction cannot win',(await state()).mode==='lost');}
