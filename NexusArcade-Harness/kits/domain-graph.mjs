...
export function pilotDomainGraph(c){
 ...
 if(c.kind==='courier'){add('delivery','delivery',{nodes:c.nodes,range:2});bindAction('delivery');if(c.spatialWorld){add('requirements','all',{});wire('delivery','complete','requirements','values');for(const item of [...new Set(c.spatialWorld.solids.filter(s=>s.openWhen).map(s=>s.openWhen.instance.slice(7)))]){const name='unlock-'+item;add(name,'contains',{item});wire('delivery','completed',name,'values');wire(name,'active','requirements','values');}wire('requirements','complete','objective','complete');}else wire('delivery','complete','objective','complete');}
