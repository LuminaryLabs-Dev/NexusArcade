...
export function pilotDomainGraph(c){
 ...
 if(c.kind==='transfer'){add('delivery','delivery',{nodes:c.nodes,range:2});bindAction('delivery');if(c.spatialWorld){add('requirements','all',{});wire('delivery','complete','requirements','values');for(const item of [...new Set(c.spatialWorld.solids.filter(s=>s.openWhen).map(s=>s.openWhen.instance.slice(7)))]){const name='unlock-'+item;add(name,'contains',{item});wire('delivery','completed',name,'values');wire(name,'active','requirements','values');}wire('requirements','complete','objective','complete');}else wire('delivery','complete','objective','complete');}
  ...
  const ranges=new Map(c.nodes.map(n=>[n.id,2.5])),ordered=[...c.nodes].sort((a,b)=>Math.min(...c.nodes.filter(o=>o!==b).map(o=>distance({x:b.x,z:b.z},{x:o.x,z:o.z})),Infinity)-Math.min(...c.nodes.filter(o=>o!==a).map(o=>distance({x:a.x,z:a.z},{x:o.x,z:o.z})),Infinity)||b.id.localeCompare(a.id));for(const n of ordered){const canExpand=c.nodes.filter(o=>o!==n).every(o=>distance({x:n.x,z:n.z},{x:o.x,z:o.z})>=3+(ranges.get(o.id)??2.5));if(canExpand)ranges.set(n.id,3);}for(const n of c.nodes){add(n.id,'valve',{id:n.id,position:{x:n.x,z:n.z},initial:n.rotation,target:n.target,range:ranges.get(n.id)});bindAction(n.id);}
