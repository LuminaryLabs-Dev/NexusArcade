...
export function createPilotEngine(c,{bestSeconds=null}={}){
 ...
 const supported=c.kind==='rally'?(road?distanceToTrack(road,p)<c.track.width/2:Math.abs(Math.hypot(p.x,p.z)-18)<4):Math.abs(p.x)<14&&Math.abs(p.z)<14&&!(c.kind==='courier'&&Math.abs(p.x)>3&&Math.abs(p.x)<7&&Math.abs(p.z)<5);
