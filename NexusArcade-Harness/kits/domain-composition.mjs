...
const positioned=new Set(['valve','delivery','checkpoints','destination']);
...
export function compileDomainComposition(composition){
 ...
 const provenance=[],instances=composition.domainInstances.map(n=>{
  ...
  if(['valve','destination'].includes(n.capabilityId))settings.position=transform(settings.position);
