...
export async function queueSnapshot(){const bundle=await contracts(),{queue,hash}=bundle,state=await loadJSON(path.join(campaign,'factory-queue.json'),{contractHash:hash,goals:{}});if(state.contractHash===hash&&state.catalogUpdates?.length)await (await import('./catalog-update.mjs')).verifyCatalogUpdateHistory(state,bundle);return {...state,contractHash:hash,goals:queue.goals.map(g=>({...g,status:state.contractHash!==hash&&state.goals[g.id]?'blocked':state.goals[g.id]?.status??'planned',execution:state.goals[g.id]??null}))};}
export async function recordGoal(id,status,{evidence=[],findings=[],checks=[]}={}){return serialized('.factory-queue',async()=>{
 const bundle=await contracts(),{queue,policy,hash}=bundle,goal=queue.goals.find(g=>g.id===id);if(!goal)throw Error('Unknown goal');
 ...
 if(state.contractHash===hash&&state.catalogUpdates?.length)await (await import('./catalog-update.mjs')).verifyCatalogUpdateHistory(state,bundle);
