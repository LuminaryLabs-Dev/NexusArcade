...
export async function recordCandidateReview(input){
 ...
 for(const review of input.candidates){
  ...
  const facets={};for(const facet of FACETS){const item=review[facet];if(!item||!verdicts.has(item.verdict)||!Array.isArray(item.evidenceRefs)||!item.evidenceRefs.length)throw Error(`Review facet ${facet} needs verdict and evidence: ${review.id}`);if(item.evidenceRefs.some(ref=>ref?.independent!==true))throw Error(`Review facet ${facet} needs explicitly independent evidence: ${review.id}`);if(facet==='comprehension'&&item.observerType!=='human')throw Error(`Comprehension review needs a human observer: ${review.id}`);if(facet==='targetDevice'&&(typeof item.deviceProfile!=='string'||!item.deviceProfile.trim()))throw Error(`Target-device review needs a device profile: ${review.id}`);for(const ref of item.evidenceRefs)await verifyEvidence(ref);facets[facet]={verdict:item.verdict,evidenceRefs:item.evidenceRefs,...(facet==='comprehension'?{observerType:item.observerType}:{}),...(facet==='targetDevice'?{deviceProfile:item.deviceProfile.trim()}: {})};}reviews.push({id:review.id,sourceHash:candidate.sourceHash,signature:candidate.signature,facets});
 }
