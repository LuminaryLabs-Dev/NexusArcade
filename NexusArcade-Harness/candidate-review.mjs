...
export async function recordCandidateReview(input){
 ...
 for(const review of input.candidates){
  ...
  const facets={};for(const facet of FACETS){const item=review[facet];if(!item||!verdicts.has(item.verdict)||!Array.isArray(item.evidenceRefs)||!item.evidenceRefs.length)throw Error(`Review facet ${facet} needs verdict and evidence: ${review.id}`);for(const ref of item.evidenceRefs)await verifyEvidence(ref);facets[facet]={verdict:item.verdict,evidenceRefs:item.evidenceRefs};}reviews.push({id:review.id,sourceHash:candidate.sourceHash,signature:candidate.signature,facets});
 }
