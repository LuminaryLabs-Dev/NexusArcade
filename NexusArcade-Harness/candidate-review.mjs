...
export async function recordCandidateReview(input){
 ...
 if(candidate.sourceHash!==currentHash)throw Error('Review candidate source is stale: '+review.id);
  const facets={};for(const facet of FACETS){const item=review[facet];if(!item||!verdicts.has(item.verdict)||!Array.isArray(item.evidenceRefs)||!item.evidenceRefs.length)throw Error(`Review facet ${facet} needs verdict and evidence: ${review.id}`);if(item.evidenceRefs.some(ref=>ref?.independent!==true))throw Error(`Review facet ${facet} needs explicitly independent evidence: ${review.id}`);if(item.evidenceRefs.some(ref=>{const evidencePath=String(ref?.path??'');return evidencePath.startsWith(review.id+'/')||evidencePath.endsWith('goals/G02/candidate-index.json')||!evidencePath.includes('campaigns/reliable-arcade-factory/goals/G03/')||!/(?:^|\/)independent(?:-|\/)/i.test(evidencePath);}))throw Error(`Review facet ${facet} must cite a named G03 independent evidence artifact: ${review.id}`);if(item.evidenceRefs.some(ref=>/replace-with|path\/to/i.test(String(ref?.path??''))))throw Error(`Review facet ${facet} contains placeholder evidence: ${review.id}`);if(facet==='comprehension'&&item.observerType!=='human')throw Error(`Comprehension review needs a human observer: ${review.id}`);if(facet==='targetDevice'&&(typeof item.deviceProfile!=='string'||!item.deviceProfile.trim()))throw Error(`Target-device review needs a device profile: ${review.id}`);if(facet==='targetDevice'&&/replace-with|path\/to/i.test(item.deviceProfile))throw Error(`Target-device review contains placeholder device profile: ${review.id}`);for(const ref of item.evidenceRefs){await verifyEvidence(ref);const evidencePath=String(ref.path);if(evidencePath.endsWith('.json')){const evidence=JSON.parse(await readFile(path.join(storage,evidencePath),'utf8'));if(evidence.sourceHash&&evidence.sourceHash!==currentHash)throw Error(`Review evidence source is stale: ${evidencePath}`);}}facets[facet]={verdict:item.verdict,evidenceRefs:item.evidenceRefs,...(facet==='comprehension'?{observerType:item.observerType}:{}),...(facet==='targetDevice'?{deviceProfile:item.deviceProfile.trim()}: {})};}reviews.push({id:review.id,sourceHash:candidate.sourceHash,signature:candidate.signature,facets});
 }
