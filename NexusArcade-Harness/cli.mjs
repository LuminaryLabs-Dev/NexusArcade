...
}else if(command==='g03-readiness'){const {writeG03Readiness}=await import('./g03-readiness.mjs');console.log(JSON.stringify(await writeG03Readiness(),null,2));
}else if(command==='g02-evidence'){const {writeG02Evidence}=await import('./g02-evidence.mjs');console.log(JSON.stringify(await writeG02Evidence(),null,2));
