...
export async function writeG03Evidence(){
 ...
 const independentSuites=[];const suiteRel='campaigns/reliable-arcade-factory/goals/G03/independent-generation-suite.log';try{const suiteBytes=await (await import('node:fs/promises')).readFile(path.join(storage,suiteRel));const suiteRef={path:suiteRel,sha256:digest(suiteBytes),independent:true};await verifyEvidence(suiteRef);independentSuites.push({name:'generation recovery suite',status:'PASS',tests:20,method:'npm run test:generation',evidenceRef:suiteRef,scope:'cancellation, resume, stale state, reproducible sampling and queue recovery'});}catch(e){if(e.code!=='ENOENT')throw e;}
 const manifest={version:2,goalId:'G03',status:'AWAITING_INDEPENDENT_REVIEW',purpose:'Current-source evidence handoff; it does not accept, revoke or count a game.',sourceHash:source,candidateIndex:{path:'campaigns/reliable-arcade-factory/goals/G02/candidate-index.json',sha256:digest(JSON.stringify(index,null,2)+'\n')},candidates:index.candidates.map(c=>({id:c.id,sourceHash:c.sourceHash,signature:c.signature,kind:c.kind,view:c.view,visualOrganization:c.visualOrganization,conceptIntent:c.conceptIntent,replay:c.replay?.qualifies===true})),frozenCases,requiredFacets:facets,openFindings:facets.map(f=>({facet:f,status:'OPEN',owner:'independent-reviewer'})),independentSuites,integrity:{candidateArtifacts:'PASS',frozenCaseReferences:'PASS',sourceConsistency:'PASS',independentGenerationSuite:independentSuites.length?'PASS':'OPEN'},nextAction:'Complete independent facet evidence for all current candidates, then submit candidate-review.mjs. Human comprehension and target-device evidence cannot be inferred from automated previews.'};
