...
import {hasBlockedText,assertAllowedText} from './text-policy.mjs';
...
const policyRevision=digest(await readFile(new URL('./text-policy.mjs',import.meta.url)));
...
export async function indexPreview(id,classification='development'){
 ...
 const index=await loadJSON(file,{version:1,revision:0,games:[]});
 const row={id,title:s.composition?.title??s.id,instructions:s.composition?.instructions??s.composition?.goal??'',controls:s.composition?.controls??'WASD / arrows + E',concepts:s.composition?.concepts??[],actions:s.composition?.version===4?[...new Set(s.composition.runtime.domainGraph.instances.map(n=>({valve:'turn valves',delivery:'carry and place',checkpoints:'reach checkpoints',flowStore:'manage flow'}[n.capability])).filter(Boolean))]:s.composition?.kind==='rally'?['steer','brake','checkpoints']:s.composition?.kind==='conduit'?['turn valves','connect flow']:s.composition?.kind==='transfer'?['carry','deliver']:[...new Set(s.composition?.nodes.map(n=>n.action)??[])],classification,artifactHash:s.artifactHash,sourceHash:s.sourceHash,started:s.started};
 assertAllowedText({title:row.title,instructions:row.instructions,controls:row.controls,concepts:row.concepts,actions:row.actions});
...
export async function library({query='',offset=0,limit=24}={}){
 ...
 const visible=rows.filter(g=>!hasBlockedText({title:g.title,instructions:g.instructions,controls:g.controls,concepts:g.concepts,actions:g.actions}));
 const needle=query.trim().toLowerCase(),filtered=visible.filter(g=>!needle||[g.title,g.instructions,...g.concepts??[],...g.actions??[]].join(' ').toLowerCase().includes(needle));
 return {revision:preview.revision+':'+accepted.revision+':'+policyRevision,total:filtered.length,accepted:active.length,previewCount:visible.filter(g=>g.classification!=='accepted').length,offset,limit,games:filtered.slice(offset,offset+limit).map(g=>({id:g.id,title:g.title,instructions:g.instructions,controls:g.controls,concepts:g.concepts,actions:g.actions,classification:g.classification})),next:offset+limit<filtered.length?offset+limit:null};
}
