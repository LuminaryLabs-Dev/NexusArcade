...
export async function measureG03Performance(){
 ...
 const results=[];try{for(const c of index.candidates){const page=await browser.newPage({viewport:{width:1100,height:780}});page.setDefaultTimeout(15000);const origin='http://127.0.0.1:'+server.address().port;await page.goto(`${origin}/${c.id}/index.html`);await page.waitForFunction(()=>!!window.render_game_to_text);await page.click('#start');await page.evaluate(()=>advanceTime(0));const sample=await page.evaluate(async()=>{const times=[];for(let i=0;i<121;i++)times.push(await new Promise(requestAnimationFrame));const intervals=times.slice(1).map((t,i)=>t-times[i]).filter(Number.isFinite).sort((a,b)=>a-b);const percentile=p=>intervals[Math.min(intervals.length-1,Math.floor((intervals.length-1)*p))]??null;return {render:window.__renderEvidence(),frameCount:intervals.length,frameIntervalMedianMs:percentile(.5),frameIntervalP95Ms:percentile(.95),animationMs:intervals.reduce((a,b)=>a+b,0),memoryBytes:performance.memory?.usedJSHeapSize??null};});results.push({id:c.id,deviceProfile:'Chromium 1100x780 headless SwiftShader reference',viewport:{width:1100,height:780},...sample});await page.close();}}finally{await browser.close();server.close();}
