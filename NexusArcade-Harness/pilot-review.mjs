...
export async function reviewPilot(root,id,{signal}={}){
 ...
 if(start.kind==='transfer'){
  ...
  await walkValve(n);while((await state()).rotation[index]!==0){await input(['KeyE']);await tick(50);}check('route valve opens '+selected,(await state()).domainState[n.id].aligned);
