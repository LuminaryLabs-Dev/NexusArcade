...
export async function reviewPilot(root,id,{signal}={}){
 const server=await serveFiles(root),report={status:'FAIL',method:'Keyboard-controlled accelerated Chromium software-WebGL pilot playthrough',checks:[],errors:[]};let browser,p,abort;const check=(name,pass,observed)=>{report.checks.push({name,pass,observed});if(!pass)throw Error(name);};
 try{const headed=process.env.NEXUS_REVIEW_HEADED==='1';browser=await chromium.launch({headless:!headed,args:['--use-angle=swiftshader','--enable-unsafe-swiftshader']});signal?.throwIfAborted();abort=()=>browser.close().catch(()=>{});signal?.addEventListener('abort',abort,{once:true});p=await browser.newPage({viewport:{width:1100,height:780}});p.setDefaultTimeout(15000);p.on('pageerror',e=>report.errors.push(e.message));const origin='http://127.0.0.1:'+server.address().port;await p.route('**/*',r=>r.request().url().startsWith(origin+'/')?r.continue():r.abort());await p.goto(origin+'/'+id+'/index.html');await p.waitForFunction(()=>!!window.render_game_to_text);const state=()=>p.evaluate(()=>JSON.parse(render_game_to_text())),tick=ms=>p.evaluate(ms=>advanceTime(ms),ms);const input=async(keys,ms=100)=>{signal?.throwIfAborted();for(const k of keys)await p.keyboard.down(k);await tick(ms);for(const k of keys)await p.keyboard.up(k);};await tick(0);check('title', (await state()).mode==='title');await p.click('#start');await tick(0);await p.check('#sound');
