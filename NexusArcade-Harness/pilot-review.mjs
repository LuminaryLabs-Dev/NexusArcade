...
export async function reviewPilot(root,id,{signal}={}){
 ...
 const walkRoute=async(x,z)=>{const initial=await state(),route=routeInWorld(profile.spatialWorld,initial.player,{x,z},initial.domainState);if(process.env.NEXUS_REVIEW_TRACE==='1')console.error(`[review:${id}] route ${initial.player.x},${initial.player.z} -> ${x},${z} points=${route.points.length}`);for(const [pointIndex,point] of route.points.slice(1).entries()){let reached=false;for(let i=0;i<700;i++){if(process.env.NEXUS_REVIEW_TRACE==='1'&&i%100===0)console.error(`[review:${id}] waypoint ${pointIndex} iter ${i}`);const s=await state(),dx=point.x-s.player.x,dz=point.z-s.player.z;if(Math.hypot(dx,dz)<.8){reached=true;break;}if(s.mode!=='play')throw Error('Route ended in '+s.mode);await input([Math.abs(dx)>.1?(dx>0?'KeyD':'KeyA'):null,Math.abs(dz)>.1?(dz>0?'KeyS':'KeyW'):null].filter(Boolean),60);}if(!reached)throw Error('World route stalled');}return route.distance;};
