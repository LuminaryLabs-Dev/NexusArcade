...
export async function reviewPilot(root,id,{signal}={}){
 const server=await serveFiles(root),report={status:'FAIL',method:'Keyboard-controlled accelerated Chromium software-WebGL pilot playthrough',checks:[],errors:[]};let browser,p,abort;const check=(name,pass,observed)=>{report.checks.push({name,pass,observed});if(process.env.NEXUS_REVIEW_TRACE==='1')console.error(`[review:${id}] ${pass?'PASS':'FAIL'} ${name}`);if(!pass)throw Error(name);};
 try{const headed=process.env.NEXUS_REVIEW_HEADED==='1';browser=await chromium.launch({headless:!headed,args:['--use-angle=swiftshader','--enable-unsafe-swiftshader']});signal?.throwIfAborted();abort=()=>browser.close().catch(()=>{});signal?.addEventListener('abort',abort,{once:true});p=await browser.newPage({viewport:{width:1100,height:780}});p.setDefaultTimeout(15000);p.on('pageerror',e=>report.errors.push(e.message));const origin='http://127.0.0.1:'+server.address().port;await p.route('**/*',r=>r.request().url().startsWith(origin+'/')?r.continue():r.abort());await p.goto(origin+'/'+id+'/index.html');await p.waitForFunction(()=>!!window.render_game_to_text);const state=()=>p.evaluate(()=>JSON.parse(render_game_to_text())),tick=ms=>p.evaluate(ms=>advanceTime(ms),ms);const input=async(keys,ms=100)=>{signal?.throwIfAborted();await p.evaluate(({keys,ms})=>window.__testInput(keys,ms),{keys,ms});};await tick(0);check('title', (await state()).mode==='title');await p.click('#start');await tick(0);await p.check('#sound');
 ...
 const walkPath=async(route,tolerance=.8,finalTolerance=tolerance)=>{for(const [pointIndex,point] of route.points.slice(1).entries()){if(process.env.NEXUS_REVIEW_TRACE==='1')console.error(`[review:${id}] waypoint ${pointIndex}`);const result=await p.evaluate(({point,tolerance})=>window.__testWalk(point,700,tolerance),{point,tolerance:pointIndex===route.points.slice(1).length-1?finalTolerance:tolerance});if(!result.reached)throw Error('World route stalled');if((await state()).mode!=='play')throw Error('Route ended in '+(await state()).mode);}};
 const walkRoute=async(x,z,tolerance=.8,finalTolerance=tolerance)=>{const initial=await state(),route=routeInWorld(profile.spatialWorld,initial.player,{x,z},initial.domainState);if(process.env.NEXUS_REVIEW_TRACE==='1')console.error(`[review:${id}] route ${initial.player.x},${initial.player.z} -> ${x},${z} points=${route.points.length}`);await walkPath(route,tolerance,finalTolerance);return route.distance;};
 ...
 if(start.kind==='transfer'){
  ...
  await walkValve(n);while((await state()).rotation[index]!==0){await input(['KeyE']);await tick(50);}if(process.env.NEXUS_REVIEW_TRACE==='1')console.error(`[review:${id}] valve ${n.id} position ${JSON.stringify((await state()).player)}`);check('route valve opens '+selected,(await state()).domainState[n.id].aligned);
  await walkRoute(selector.x,selector.z+1.5);while((await state()).rotation[0]!==Number(conservative)){await input(['KeyE']);await tick(50);}await tick(4000);const flowing=await state();check('selected route carries flow '+selected,flowing.domainState[selected].rate>0&&flowing.domainState[selected==='direct'?'efficient':'direct'].rate===0);check('target and waste accumulate',flowing.fill>0&&flowing.domainState.waste.volume>0);
  ...
  for(;ticks<4000;ticks++){
   ...
   if(!roadContains(main,next.player)){offMainTicks++;if(!fault&&!conservative&&!report.image){report.image=await p.screenshot();report.frame=next;if((start.kind==='rally'||start.kind==='checkpoint'))report.detailImage=await p.screenshot({clip:{x:120,y:440,width:860,height:280}});}}
 ...
 await tick(301000);check('idle failure',(await state()).mode==='lost');await p.keyboard.press('KeyR');await tick(0);check('restart after failure',(await state()).mode==='play');check('no browser errors',report.errors.length===0);report.status='PASS';if(report.image)report.screenshotHash=digest(report.image);if(report.detailImage)report.detailHash=digest(report.detailImage);signal?.removeEventListener('abort',abort);
  ...
  report.errors.push(e.message);report.failure={afterCheck:report.checks.at(-1)?.name??'startup'};
  if(p&&!p.isClosed()&&!signal?.aborted){let timer;try{
   await Promise.race([(async()=>{if(!report.image){report.image=await p.screenshot({timeout:2500});report.failure.image='review.png';}
