# G24 — Expressive feedback: Review

## Summary
Give each game responsive animation, audio and effects suited to its rules.

## Outcome
Independent B07 verdict and regression findings for affected earlier batches.

## Changes
- Required work: Test timing, interrupted feedback, stacking effects, silent operation, audio overlap, sensory overload and frame-time cost.
- Entry: G23 completed with current evidence. Follow [shared rules](../RULES.md); queue fields are indexed in ../goal-queue.json.
...
Execution authority: [goal-queue.json](../goal-queue.json) entry G24 and [execution-contract.json](../execution-contract.json).
