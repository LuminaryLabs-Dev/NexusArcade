# G01 — Factory foundation: Plan

## Summary
Establish the queue, versioned catalogs, profile compiler, recursive improvement controller and usable arcade.
...
## Changes
- Required work: Audit current fixed composition, 240-second timer, call/token quotas and narrow PASS checks. Define queue state transitions, admission, cancellation and evidence contracts.
- Entry: packet rules resolved and source audit available. Follow [shared rules](../RULES.md); queue fields are indexed in ../goal-queue.json.
...
Execution authority: [goal-queue.json](../goal-queue.json) entry G01 and [execution-contract.json](../execution-contract.json).
...
## Review and Gates
- Specific gate: No batch admission until all foundation gates pass; old PASS records keep their original rubric.
...
- Status: in progress. Foundation probes and an execution packet exist under the campaign G01 directory; mandatory calibration and compiler integration remain unresolved. This outline is not runtime proof.
